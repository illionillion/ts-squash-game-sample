...
// 変数の宣言
let ballX : number = 600 //ボールX座標
let ballY : number = 300 //ボールY座標
let ballXp :number = 0 //ボールX軸への速さ
let ballYp :number = 0 //ボールY軸への速さ
let barX : number = 600 //バーX
let barY : number = 700 //バーY
let score : number = 0
let scene : number = 0
...
const mainloop = () => {
    ...
    if (scene === 0) { //タイトル
        ...
        ballX += ballXp
        ballY += ballYp
        if(ballX <= 260 || ballX >= 940) ballXp = -ballXp //X軸 反転
        if(ballY <= 60 /*|| ballY >= 790*/) ballYp = 8 + rnd(8)//-ballYp //Y軸 反転
        if(ballY > 800) scene = 2
        // sCir(ballX, ballY, 10 , "lime")
    
        barX = tapX //マウスポインタの位置
        if (barX < 300) barX = 300 //バー(左)位置固定
        if (barX > 900) barX = 900 //バー(右)位置固定
